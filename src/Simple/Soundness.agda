...
eval-soundness {Σₕ = Σₕ}{Σₛ = Σₛ}{𝓢 = 𝓢} ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft wf-𝓗 wf-𝓢 ≤-q (TAbs {q = 𝟙} ⊢e qbdd {cq₁}{cq₂}) (EAbs cqc₁ cqc₂ refl qbe)
  = record
      { ↑Σₕ′ = Σₕ
      ; ↑Σ≼ₕ = ≼-refl
      ; ↑Σₛ′ = Σₛ
      ; ↑Σ≼ₛ = ≼-refl
      ; ↑⊢v = TVClos (λ 𝓢′ 𝓢≼ → ⊨-adjust-≼ₛ 𝓢≼ (⊨-qbe qbe (is-bounded qbdd) (restrict′ ⊨𝓔 qbdd))) refl (is-bounded qbdd) ⊢e cq₁ cq₂ <⦂-refl
      ; ↑⊢𝓗 = ⊢𝓗
      ; ↑⊢𝓢 = ⊢𝓢
      ; ↑𝓢≼ = ≼ₛ-refl {𝓢}
...
eval-soundness {Σₕ = Σₕ}{Σₛ = Σₛ}{𝓢 = 𝓢} ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft wf-𝓗 wf-𝓢 ≤-refl (TAbs {q = 𝟚} ⊢e qbdd {cq₁}{cq₂}) (EAbs ≤-refl cqc₂ refl qbe)
  ...
  = record
      ...
      ; ↑Σₛ′ = Σₛ
      ; ↑Σ≼ₛ = ≼-refl
      ; ↑⊢v = TVClos (λ 𝓢′ 𝓢≼ → ⊨-adjust-≼ₛ 𝓢≼ (restrict′ ⊨𝓔 qbdd)) refl (is-bounded qbdd) ⊢e cq₁ cq₂ <⦂-refl
      ; ↑⊢𝓗 = ⊢𝓗
      ; ↑⊢𝓢 = ⊢𝓢
      ; ↑𝓢≼ = ≼ₛ-refl {𝓢}
...
... | ih@record { ↑Σₕ′ = ↑Σₕ′₁ ; ↑Σₛ′ = ↑Σₛ′₁ ; ↑Σ≼ₕ = ↑Σ≼ₕ₁ ; ↑Σ≼ₛ = ↑Σ≼ₛ₁ ; ↑⊢v = TVClos {x = X s _} {S₁≤x = S₁≤x} ∀⊨𝓔 refl qbd ⊢e₂ cq₁ ≤-refl (SQual qsub (SFun {S₃ = S₃}{S₁ = S₁@ (T₁ ^ 𝟙)}{S₂ = S₂@ (T₂ ^ 𝟙)}{S₄ = S₄} S₁<⦂S₂ S₃<⦂S₄)) ; ↑⊢𝓗 = ↑⊢𝓗₁ ; ↑⊢𝓢 = ↑⊢𝓢₁ ; ↑𝓢≼ = ↑𝓢≼₁ ; ↑cls = ↑cls₁ ; ↑wf-𝓗 = ↑wf-𝓗₁ ; ↑wf-𝓢 = ↑wf-𝓢₁ }
  with eval-soundness (↑⊢𝓗 ih) (↑⊢𝓢 ih)
                      (⊨-extend (↑Σ≼ₕ ih) (↑Σ≼ₛ ih) (↑𝓢≼ ih) ⊨𝓔)
                      (wfe-ext-≼ₛ (↑𝓢≼ ih) 𝓔-wft)
                      (↑wf-𝓗 ih)
                      (↑wf-𝓢 ih)
                      (≤-trans (q-of-mono S₁<⦂S₂) cq₁)
                      ⊢e₁ ⇓₁
...
eval-soundness ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft wf-𝓗 wf-𝓢 ≤-q (TApp ⊢e ⊢e₁) (EApp {𝓢 = 𝓢}{q = 𝟚} {q₁ = 𝟙}{𝓢′ = 𝓢′}{𝓢″ = 𝓢″}{𝓢‴ = 𝓢‴} q₂≤q ⇓ ⇓₁ ⇓₂ refl)
  with eval-soundness ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft  wf-𝓗 wf-𝓢 ≤-top ⊢e ⇓
... | ih@record { ↑Σₕ′ = ↑Σₕ′₁ ; ↑Σₛ′ = ↑Σₛ′₁ ; ↑Σ≼ₕ = ↑Σ≼ₕ₁ ; ↑Σ≼ₛ = ↑Σ≼ₛ₁ ; ↑⊢v = TVClos {𝓢 = 𝓢ᶜ} {x = X s _} {S₁≤x = refl} ∀⊨𝓔 𝓢≡ qbd ⊢e₂ cq₁ cq₂ (SQual qsub (SFun {S₃ = S₃}{S₁ = S₁@ (T₁ ^ 𝟙)}{S₂ = S₂}{S₄ = S₄} S₁<⦂S₂ S₃<⦂S₄)) ; ↑⊢𝓗 = ↑⊢𝓗₁ ; ↑⊢𝓢 = ↑⊢𝓢₁ ; ↑𝓢≼ = ↑𝓢≼₁ ; ↑cls = ↑cls₁ ; ↑wf-𝓗 = ↑wf-𝓗₁ ; ↑wf-𝓢 = ↑wf-𝓢₁ }
  with eval-soundness (↑⊢𝓗 ih) (↑⊢𝓢 ih)
                      (⊨-extend (↑Σ≼ₕ ih) (↑Σ≼ₛ ih) (↑𝓢≼ ih) ⊨𝓔)
                      (wfe-ext-≼ₛ (↑𝓢≼ ih) 𝓔-wft)
                      (↑wf-𝓗 ih)
...
eval-soundness ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft wf-𝓗 wf-𝓢 ≤-q (TApp ⊢e ⊢e₁) (EApp {𝓢 = 𝓢}{q = 𝟚} {q₁ = 𝟚}{𝓢′ = 𝓢′}{v₂ = v₂}{𝓢″ = 𝓢″}{𝓢‴ = 𝓢‴} q₂≤q ⇓ ⇓₁ ⇓₂ refl)
  with eval-soundness ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft  wf-𝓗 wf-𝓢 ≤-top ⊢e ⇓
... | ih@record { ↑Σₕ′ = ↑Σₕ′₁ ; ↑Σₛ′ = ↑Σₛ′₁ ; ↑Σ≼ₕ = ↑Σ≼ₕ₁ ; ↑Σ≼ₛ = ↑Σ≼ₛ₁ ; ↑⊢v = TVClos {𝓢 = 𝓢ᶜ} {x = X s _} {S₁≤x = S₁≤x} ∀⊨𝓔 𝓢≡ qbd ⊢e₂ cq₁ cq₂ (SQual qsub (SFun {S₃ = S₃}{S₁ = S₁}{S₂ = S₂}{S₄ = S₄} S₁<⦂S₂ S₃<⦂S₄)) ; ↑⊢𝓗 = ↑⊢𝓗₁ ; ↑⊢𝓢 = ↑⊢𝓢₁ ; ↑𝓢≼ = ↑𝓢≼₁ ; ↑cls = ↑cls₁ ; ↑wf-𝓗 = ↑wf-𝓗₁ ; ↑wf-𝓢 = ↑wf-𝓢₁ }
  with eval-soundness (↑⊢𝓗 ih) (↑⊢𝓢 ih)
                      (⊨-extend (↑Σ≼ₕ ih) (↑Σ≼ₛ ih) (↑𝓢≼ ih) ⊨𝓔)
                      (wfe-ext-≼ₛ (↑𝓢≼ ih) 𝓔-wft)
                      (↑wf-𝓗 ih)
...
eval-soundness ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft wf-𝓗 wf-𝓢 ≤-q (TRef {S = S} {cq = ≤-refl} ⊢e) (ERef {q = 𝟙}{𝓢 = 𝓢}{𝓢′ = 𝓢′} ≤-refl ⇓ (refl , refl , refl))
  with eval-soundness ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft wf-𝓗 wf-𝓢 ≤-refl ⊢e ⇓
... | ih
...
eval-soundness {q = 𝟚} ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft wf-𝓗 wf-𝓢 ≤-q (TRef {S = S} {cq = ≤-bottop} ⊢e) (ERef {q₁ = q₁} {q = q} {𝓢 = 𝓢} {𝓢′ = 𝓢′} x ⇓ (refl , refl))
...
eval-soundness {q = 𝟚} ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft wf-𝓗 wf-𝓢 ≤-q (TRef {S = S@ (mkQ 𝟙 T)} {cq = ≤-refl} ⊢e) (ERef {q₁ = q₁} {q = q} {𝓢 = 𝓢} {𝓢′ = 𝓢′} x ⇓ (refl , refl , refl))
  with eval-soundness ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft wf-𝓗 wf-𝓢 ≤-refl ⊢e ⇓
... | ih
  rewrite sym (⊢ₕ-length (ih .↑⊢𝓗))
  = record
...
eval-soundness {q = 𝟚} ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft wf-𝓗 wf-𝓢 ≤-q (TRef {S = S@ (mkQ 𝟚 T)} {cq = ≤-refl} ⊢e) (ERef {q₁ = q₁} {q = q} {𝓢 = 𝓢} {𝓢′ = 𝓢′} x ⇓ (refl , refl))
  with eval-soundness ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft wf-𝓗 wf-𝓢 ≤-refl ⊢e ⇓
... | ih
  rewrite sym (⊢ₛ-length {𝓢 = 𝓢′} (ih .↑⊢𝓢))
  = record
...
eval-soundness ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft wf-𝓗 wf-𝓢 ≤-q (TDeref {cq = cq} ⊢e) (EDeref {q₁ = 𝟙} ⇓ hread)
  with eval-soundness ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft wf-𝓗 wf-𝓢 ≤-top ⊢e ⇓
...
eval-soundness ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft wf-𝓗 wf-𝓢 ≤-q (TDeref {cq = cq} ⊢e) (EDeref {q₁ = 𝟚} ⇓ xread)
  with eval-soundness ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft wf-𝓗 wf-𝓢 ≤-top ⊢e ⇓
